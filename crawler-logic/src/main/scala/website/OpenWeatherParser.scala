...
object OpenWeatherParser extends Logging {
		...
		data: (String, String, String, String, List[Map[String, String]])) = 
	{
		...
		// 
		val smbChar = iconMap.getOrElse(today("smbVar"), 
			"b")
		...
		sb.append("°C").append("\n")
		// 
		sb.append(today("psuVal")).append(today(
			"psuUnt")).append("\n")
		// 
		sb.append(today("wndMps")).append(
			"Mps").append("\n")
		val wndArw = arrowMap.getOrElse(today(
			"wndDct"), " ")
		sb.append(wndArw).append("\n")

		val l7 = for (i <- 1 to 4) yield getDayOfWeek(data._5(i)("day"), timeZone, local)
		sb.append(l7.mkString(
			"             ")).append("\n")

		val l8 = for (i <- 1 to 4) yield iconMap.getOrElse(data._5(i)(
			"smbVar"), "b")
		sb.append(l8.mkString(
			"  ")).append("\n")

		val l9 = for (i <- 1 to 4) yield data._5(i)("humVal") +  data._5(i)("humUnt")
		sb.append(l9.mkString(
			"  ")).append("\n")

		val l10 = for (i <- 1 to 4) yield data._5(i)("wndMps") + 
		"Mps"
		sb.append(l10.mkString(
			"         ")).append("\n")

		val l11 = for (i <- 1 to 4) yield arrowMap.getOrElse(data._5(i)("wndDct"), " ")
		sb.append(l11.mkString("\n")).append("\n")

		val l15 = for (i <- 1 to 4) yield data._5(i)("tmpDay") + "°C"
		sb.append(l15.mkString(
			"           ")).append("\n")

		sb.append(today(
			"smbNme")).append("\n")
		sb.append(today(
			"humVal")).append(today("humUnt")).append("\n")
