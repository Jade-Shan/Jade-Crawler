...
object OpenWeatherParser extends Logging {
		...
		data: (String, String, String, String, List[Map[String, String]])) = 
	{
		...
		for (i <- 1 to 4) {
			...
			if (i > 1) sb.append("%12s" format str) else sb.append("%4s" format str)
		} 
		sb.append("\n")
		...
		for (i <- 1 to 4) {
			...
			if (i == 1) sb.append("%6s" format str) else sb.append("%12s" format str)
		}
		sb.append("\n")

		for (i <- 1 to 4) {
			val str = data._5(i)("wndMps") + "Mps"
			if (i == 1) sb.append("%8s" format str) else sb.append("%12s" format str)
		}
		sb.append("\n")
		...
		for (i <- 1 to 4) {
			...
			if (i == 1) sb.append("%9s" format str) else sb.append("%11s" format str)
		}
		sb.append("\n")
...
object OpenWeatherCrawler extends Logging {
	...
	def get16dayByCityName(city: String, localTimeZone: String, apiKey: String) = {
